<?php
/**
 * AJAX: Buscar Pacientes para Psicología Clínica
 */
...
try {
    ...
    if (count($pacientes) > 0) {
        foreach ($pacientes as $p) {
            ...
            if ($p['ultima_visita_id']) {
                ...
                $html .= '    <a href="' . $project_folder . '/app/views/especialidades/psicologia.php?id_visita=' . $p['ultima_visita_id'] . '" class="btn btn-sm btn-outline-purple rounded-pill" style="border-color:#6f42c1;color:#6f42c1">';
                $html .= '      <i class="bi bi-brain"></i> Abrir Última (' . date('d/m/y', strtotime($p['ultima_visita_fecha'])) . ')';
                $html .= '    </a>';
            } else {
    ...
    error_log("Error en buscar_pacientes_psicologia.php: " . $e->getMessage());
